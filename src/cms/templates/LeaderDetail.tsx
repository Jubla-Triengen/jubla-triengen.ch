...
export default function LeaderDetailTemplate() {
  const { id } = useParams<{ id: string }>();
  const data = useCmsData();
  const page = useCmsPage("leader_detail");
  const meta = (page?.meta ?? {}) as LeaderDetailMeta;

  const leader = data.leaders.find((entry) => entry.id === id);
  ...
  return (
    <DetailPageLayout
      hero={{
        image:
          leader.backgroundImage || leader.image || page?.hero?.image || "",
        title: leader.name,
            ...
            <div className="flex flex-col gap-3 text-gray-600">
              ...
              {!leader.email && !leader.phone && (
                <div className="flex items-center gap-3 italic text-gray-400">
                  ...
                  <span className="text-sm">{meta.noContactText}</span>
                </div>
              )}
            </div>
          </div>
