import { useNavigate } from "react-router-dom";
import { ArrowLeft } from "lucide-react";
import Button from "../../components/ui/Button";
import { useCmsPage } from "../context";

interface NotFoundMeta {
  code?: string;
  title?: string;
  description?: string;
  buttonText?: string;
  backgroundImage?: string;
}

export default function NotFoundTemplate() {
  const navigate = useNavigate();
  const page = useCmsPage("not_found");
  const meta = (page?.meta ?? {}) as NotFoundMeta;

  const content = {
    code: meta.code ?? "404",
    title: meta.title ?? "Seite nicht gefunden",
    description:
      meta.description ?? "Die angeforderte Seite existiert leider nicht.",
    buttonText: meta.buttonText ?? "Zurück zur Startseite",
    backgroundImage: meta.backgroundImage ?? "",
  };

  return (
    <section className="relative h-screen flex items-center overflow-hidden">
      <div
        className="absolute inset-0 z-0"
        style={{
          backgroundImage: `url(${content.backgroundImage})`,
          backgroundSize: "cover",
          backgroundPosition: "center",
          filter: "grayscale(100%)",
        }}
      >
        <div className="absolute inset-0 bg-black/60"></div>
      </div>

      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        <div className="max-w-3xl mx-auto">
          <h1 className="font-amatic text-9xl font-bold text-jubla-yellow mb-4">
            {content.code}
          </h1>
          <h2 className="font-amatic text-5xl sm:text-6xl font-bold text-white mb-8">
            {content.title}
          </h2>
          <p className="font-mundial font-light text-xl text-gray-200 mb-12 max-w-2xl mx-auto">
            {content.description}
          </p>
          <div className="flex justify-center">
            <Button
              onClick={() => navigate("/")}
              icon={ArrowLeft}
              iconPosition="left"
            >
              {content.buttonText}
            </Button>
          </div>
        </div>
      </div>
    </section>
  );
}
